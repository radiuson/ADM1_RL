#!/usr/bin/env python3
"""
PPO-FKOnly Training: F_K shaping only (no curriculum)
======================================================
reward_config = safety_first_curriculum  (no --curriculum flag)
Model output: models/ppo_{scenario}_safety_first_curriculum_seed{seed}/
"""
...
def main():
    ...
    for i, scenario in enumerate(args.scenarios):
        for j, seed in enumerate(args.seeds):
            ...
            desc = f"PPO-FKOnly [{n}/{total}] {scenario} seed={seed}"
            model_path = ROOT / 'models' / f'ppo_{scenario}_safety_first_curriculum_seed{seed}' / 'final_model.zip'
    ...
    if failures:
        ...
        print(f"  DONE — all PPO-FKOnly runs completed successfully")
    print(f"{'='*65}")
