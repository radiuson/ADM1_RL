...
Directory layout expected under --results-dir:
    ...
    paper_direction_a_multi/evaluation/per_run/
...
def load_all(results_dir: pathlib.Path) -> dict:
    ...
    multi_per_run = results_dir / 'paper_direction_a_multi' / 'evaluation' / 'per_run'
    ...
    return {
        ...
        # ── Engineering baselines (re-evaluated with union-based MetricsCalculator) ─
        # Generated by scripts/rerun_baseline_eval.py.
        # Scenario order: nominal, high_load, shock_load, low_load,
        #                 temperature_drop, cold_winter.
        # violation_rate is union-based (∈ [0,1]), consistent with SAC results.
        'Constant': dict(
            mean=[ 0.787,  0.550,  0.789,  0.762,  0.787, -0.371],
            std=None,
            viol=[ 0.161,  0.959,  0.158,  0.000,  0.161,  0.358],
            color='#7F7F7F', lw=1.3, ls='--',  mk='o', ms=6,  hatch='//',  sac=False,
        ),
        'PID': dict(
            mean=[ 0.543,  0.620,  0.544,  0.755,  0.543, -0.433],
            std=None,
            viol=[ 0.938,  0.989,  0.938,  0.000,  0.938,  0.510],
            color='#ED7D31', lw=1.3, ls='--',  mk='s', ms=6,  hatch='\\\\', sac=False,
        ),
        'Cascaded PID': dict(
            mean=[ 0.543,  0.563,  0.545,  0.819,  0.544, -0.390],
            std=None,
            viol=[ 0.921,  0.928,  0.915,  0.000,  0.920,  0.300],
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help='Root results directory containing paper_direction_a/ and '
             'paper_direction_a_multi/ subdirectories.',
    )
    parser.add_argument(
